  ...
  <div class="space-y-8">
    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div class="space-y-2">
        ...
        <span class="block max-w-2xl text-sm text-muted-foreground sm:text-base">View live order activity, revenue, and
          recent service updates in one place.</span>
      </div>
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-4">
      <!-- Total Orders -->
      <div
        class="relative min-w-0 overflow-hidden rounded-3xl border border-border bg-card p-4 text-card-foreground shadow-sm sm:p-5">
        <div class="flex min-h-32 flex-col justify-between gap-4">
          ...
          <div class="min-w-0 space-y-1">
            ...
            <span v-else
              class="block min-w-0 truncate text-3xl font-semibold leading-none text-foreground sm:text-4xl">{{
                orders?.length || 0 }}</span>
            <span class="text-sm text-muted-foreground">{{ selected_range_label }}</span>
          </div>
        </div>
        <i
          class="pi pi-shopping-bag pointer-events-none absolute -bottom-4 right-0 text-7xl text-muted-foreground/10 sm:text-8xl"></i>
      </div>
      ...
      <div
        ...
        <div class="flex min-h-32 flex-col justify-between gap-4">
          ...
          <div class="min-w-0 space-y-1">
            ...
            <span v-else
              class="block min-w-0 break-all text-3xl font-semibold leading-none text-emerald-600 sm:text-4xl">{{
                formatCurrency(totalRevenue) }}</span>
            <span class="text-sm text-muted-foreground">{{ selected_range === 'all' ? 'Total revenue' :
              `${selected_range_label} revenue` }}</span>
          </div>
        </div>
        <i
          class="pi pi-dollar pointer-events-none absolute -bottom-4 right-0 text-7xl text-emerald-500/10 sm:text-8xl"></i>
      </div>
      ...
      <div
        ...
        <div class="flex min-h-32 flex-col justify-between gap-4">
          ...
          <div class="min-w-0 space-y-1">
            ...
            <span v-else class="block min-w-0 truncate text-3xl font-semibold leading-none text-blue-600 sm:text-4xl">{{
              active_orders?.length || 0 }}</span>
            <span class="text-sm text-muted-foreground">In progress</span>
          </div>
        </div>
        <i
          class="pi pi-spinner pointer-events-none absolute -bottom-4 right-0 text-7xl text-blue-500/10 sm:text-8xl"></i>
      </div>
      ...
      <div
        ...
        <i
          class="pi pi-check-circle pointer-events-none absolute -bottom-4 right-0 text-7xl text-emerald-500/10 sm:text-8xl"></i>
      </div>
    </div>
    ...
    <div class="space-y-4">
      <div class="rounded-3xl border border-border bg-card p-4 shadow-sm sm:p-5">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p class="text-sm text-muted-foreground">Search for a customer or narrow the time range without breaking the
              layout.</p>
          </div>

          <div class="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-end">
            <form @submit.prevent="submitCustomerSearch"
              class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
              <input v-model="customer_search_input" type="text" placeholder="Search by customer"
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <button @click="selected_range = 'all'"
              ...
              <button @click="selected_range = 'day'"
              ...
              <button @click="selected_range = 'week'"
              ...
              <button @click="selected_range = 'month'"
      ...
      <div v-if="orders?.length === 0"
        class="rounded-3xl border border-border bg-card p-10 text-center shadow-sm sm:p-12">
        <i class="pi pi-inbox text-4xl text-muted-foreground mb-4 block"></i>
        <p class="text-muted-foreground">{{ selected_range === 'all' ? 'No orders yet' : `No orders for
          ${selected_range_label.toLowerCase()}` }}</p>
      </div>
      ...
      <section v-if="orders_loading == 'success'" v-for="order in orders" :key="order.id">
        <OrderDashboardComponentsOrderDetailsCard :order_details="order"></OrderDashboardComponentsOrderDetailsCard>
      </section>
    </div>
  </div>
</template>
