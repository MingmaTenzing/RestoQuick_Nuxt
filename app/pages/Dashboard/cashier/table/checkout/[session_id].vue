...
const {
  ...
  error: sessionError,
  refresh,
} = await useFetch<TableSessionCheckoutResponse>(
  `/api/orders/checkout/table/${sessionId.value}`,
);
...
const payableOrderIds = computed(
  () => sessionSummary.value?.payableOrderIds ?? [],
);
const sessionTotalCents = computed(
  () => sessionSummary.value?.sessionTotalCents ?? 0,
);
const payableTotalCents = computed(
  () => sessionSummary.value?.payableTotalCents ?? 0,
);
const payableOrderCount = computed(
  () => sessionSummary.value?.payableOrderCount ?? 0,
);
const tenderedCents = computed(() =>
  Math.round((tenderedAmount.value ?? 0) * 100),
);
const changeDueCents = computed(() =>
  paymentMethod.value === "CASH"
    ? Math.max(tenderedCents.value - payableTotalCents.value, 0)
    : 0,
);
const canPay = computed(() => {
  if (payableOrderCount.value === 0) {
    return false;
  }
...
function addQuickAmount(amount: number) {
  tenderedAmount.value = Number(
    ((tenderedAmount.value ?? 0) + amount).toFixed(2),
  );
}
  ...
  try {
    await $fetch("/api/orders/checkout/table/mark-paid", {
      ...
      body: {
        ...
        orderIds: payableOrderIds.value,
...
<template>
  <div v-if="session" class="print-receipt-root hidden print:block">
    <div
      class="mx-auto w-[80mm] space-y-3 bg-white px-4 py-6 font-mono text-[11px] leading-[1.45] text-black"
    >
      <div class="text-center">
        <p class="text-[13px] font-bold uppercase tracking-widest">
          RestoQuick
        </p>
        ...
        <template v-for="order in allOrders" :key="order.id">
          <div>
            ...
            <div v-for="item in order.items" :key="item.id" class="mt-1 pl-2">
              ...
              <div
                v-for="option in item.orderItemOptions"
                :key="option.id"
                class="pl-3 text-black/60"
              >
                + {{ option.quantity }}x
                {{ option.menuOption?.name ?? "Option" }}
              </div>
            </div>
          </div>
        </template>
      ...
      <div class="space-y-4">
        ...
        <p v-if="session" class="text-sm text-muted-foreground">
          ...
          <NuxtTime
            ...
            minute="2-digit"
          />
          <span v-if="session.table.capacity">
            · Capacity {{ session.table.capacity }}</span
          >
        </p>
      </div>
    ...
    >
      ...
      <p class="mt-1 text-sm text-muted-foreground">
        {{
          ...
          "This session may have already been closed."
        }}
      </p>
      <NuxtLink
        to="/dashboard/cashier"
    ...
    <div v-else class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
      <div class="space-y-4">
        <section class="rounded-3xl border border-border bg-card">
          <div class="flex items-center justify-between gap-4 px-6 py-5">
            <div>
              <p
                class="text-[11px] font-medium uppercase tracking-widest text-muted-foreground"
              >
                Session orders
              </p>
              <h2 class="mt-0.5 text-lg font-semibold text-foreground">
              ...
              <div class="flex flex-wrap items-start justify-between gap-2">
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    ...
                    :class="
                      statusClasses[order.status] || 'text-muted-foreground'
                    "
                  >
                    {{
      ...
      <div class="space-y-4">
        <section class="rounded-4xl border border-border bg-card p-5 md:p-6">
          ...
          <h2
            class="mt-1 text-xl font-medium tracking-tight text-foreground/70"
          >
            Collect payment
          </h2>

          <div
            class="mt-5 rounded-3xl border border-border bg-secondary px-5 py-4 shadow-sm"
          >
            <p
              class="text-xs font-semibold uppercase tracking-widest text-muted-foreground"
            >
            ...
            <p
              class="mt-1 text-4xl font-semibold tracking-tight text-foreground"
            >
              $ {{ payableTotalCents / 100 }}
            </p>
            <p class="mt-1 text-sm text-muted-foreground">
              {{ payableOrderCount }} payable order{{
                payableOrderCount !== 1 ? "s" : ""
              }}
            </p>
          </div>
          ...
          <div v-if="paymentMethod === 'CASH'" class="mt-5 space-y-4">
            ...
            <div
              class="rounded-3xl border border-border bg-secondary px-4 py-3.5 shadow-sm"
            >
              <p
                class="text-xs font-semibold uppercase tracking-widest text-muted-foreground"
              >
              ...
              <p
                ...
                :class="
                  tenderedCents >= payableTotalCents && payableTotalCents > 0
          ...
          >
            Present the EFTPOS terminal to the customer. Confirm payment, then
            record it below.
          </div>
