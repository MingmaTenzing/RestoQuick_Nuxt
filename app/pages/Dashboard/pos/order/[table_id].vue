...
<template>
    ...
    <section class="flex h-full min-h-0 gap-4 overflow-hidden p-2 lg:p-4">
        <div class="flex min-h-0 flex-1 flex-col overflow-y-auto no-scrollbar pr-1">
            <div class="space-y-6 pb-2">

                <Pos_Order_Header :table="table" :total-items="total_item_count" :subtotal-cents="subtotal_cents"
                    :service-label="serviceLabel" :back-to="backTo" :back-label="backLabel" />
                <Pos_Filter_Bar :categories="menuCategories!" :search-query="searchQuery"
                    :selected-category="selectedCategory" @search-change="searchQuery = $event"
                    @category-change="selectedCategory = $event" />

                <section class="space-y-3">
                    <div class="flex items-end justify-between px-1">
                        <div>
                            <p class="text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground">Live
                                menu</p>
                    ...
                    <div v-if="menuPending" class="grid gap-3 md:grid-cols-3 2xl:grid-cols-3">
                        <div v-for="index in 6" :key="index"
                            class="h-64 animate-pulse rounded-3xl border border-border bg-card"></div>
                    </div>

                    <div v-else-if="filteredMenuItems.length" class="grid gap-3 md:grid-cols-3 2xl:grid-cols-3">
                        <Pos_Menu_Item_Card v-for="item in filteredMenuItems" :key="item.id" :item="item"
                            :quantity-in-cart="quantity_for_menu_item(item.id)" @add="add_to_cart" />
                    </div>
        ...
        <div class="flex h-full min-h-0 shrink-0">
            <Pos_Order_Sidebar :table="table" :cart-items="cart_items" :total-items="total_item_count"
                :subtotal-cents="subtotal_cents" :is-submitting="isSubmittingOrder" :service-label="serviceLabel"
                @increase="increase_quantity" @decrease="decrease_quantity" @remove="remove_from_cart"
                @clear="empty_cart" @submit="submitOrder" />
