...
<template>
  <section>
    <NuxtLink :to="`/dashboard/orders/${order_details.id}`" class="block">
      <div
        class="group rounded-3xl border border-border bg-card p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/20 hover:bg-accent/30 hover:shadow-md sm:p-5">
        <div class="grid gap-4 md:grid-cols-[auto_minmax(0,1fr)] xl:grid-cols-[auto_minmax(0,1fr)_auto]">
          <div class="flex items-center gap-3 md:items-start md:pr-1">
            <div
              class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-accent text-base font-semibold text-primary">
          ...
          <div class="min-w-0 space-y-3">
            ...
            <div class="flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-muted-foreground">
              <span
                class="inline-flex items-center gap-1.5 rounded-full bg-background px-3 py-1 text-xs font-medium text-muted-foreground">
          ...
          <div
            class="flex flex-col gap-3 border-t border-border pt-4 md:col-span-2 md:flex-row md:items-center md:justify-between xl:col-span-1 xl:min-w-52 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
            <div class="flex flex-wrap items-center gap-2 xl:justify-end">
              ...
              <span
                :class="['inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium', getStatusColor(order_details.status)]">
            ...
            <div class=" ">
              <p class="text-lg font-semibold text-foreground">{{ formatCurrency(order_details.totalAmountCents / 100)
              }}
              </p>
            </div>
          </div>
