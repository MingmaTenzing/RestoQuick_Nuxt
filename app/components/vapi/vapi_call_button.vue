...
<template>
  <template v-if="variant === 'button-only'">
    <div class="relative flex items-center justify-center min-w-[200px] transition-all duration-300">
      <button @click="isCalling ? endCall() : startCall()" :disabled="isLoading"
  ...
  <div v-else class="overflow-hidden rounded-4xl border border-border bg-card shadow-sm">
    <div class="relative border-b border-border px-6 py-10 sm:px-8 sm:py-12">
      ...
      <div class="relative flex flex-col items-center text-center">
        <div
          class="flex h-28 w-28 items-center justify-center rounded-full border border-green-500/30 bg-background/90 shadow-[0_0_0_14px_rgba(34,197,94,0.08)] backdrop-blur">
          <div
            class="flex h-20 w-20 items-center justify-center rounded-full bg-green-600 text-white shadow-lg shadow-green-600/30">
        ...
        <p class="mt-3 max-w-md text-sm leading-7 text-muted-foreground sm:text-base">
          Start a live conversation, let Maya collect the reservation details, and see the call status update in real
          time.
        </p>
      </div>
    </div>
    ...
    <div class="bg-background/95 p-6 sm:p-8">
      <div class="flex items-start justify-between gap-4">
        ...
        <div
          class="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1.5 text-xs font-semibold text-foreground">
          <span
            :class="['h-2.5 w-2.5 rounded-full', isCalling ? 'bg-green-500 animate-pulse' : 'bg-muted-foreground/40']"></span>
          {{ isCalling ? 'Live now' : isLoading ? 'Connecting' : 'Ready' }}
        </div>
      </div>

      <div class="mt-5 flex flex-col gap-3 sm:flex-row">
        <button @click="startCall" :disabled="isCalling || isLoading"
        ...
        <button @click="endCall" :disabled="!isCalling"
