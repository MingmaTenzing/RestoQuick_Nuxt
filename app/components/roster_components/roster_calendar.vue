...
<div class=" " >
    <div class=" flex justify-between items-center w-full my-4">
        ...
        <span class=" text-2xl font-semibold">Weekly 
            ...
            <div class=" flex gap-4 items-center">
                <button
                    class="border-border border px-4 py-2 flex justify-center items-center space-x-2 hover:scale-105 rounded-3xl transition-all ease-linear hover:border-ring"
                    @click="open"
                >
                    <i class="pi pi-sparkles"></i>
                    <span>AI Suggestions</span>
                </button>
                <button v-on:click="previousWeek()" class=" border px-4 py-2 bg-card hover:border-ring  rounded-3xl"><</button>
                <span class=" font-medium text-lg">
                    
                    {{ weekRangeText }}
                </span>
                <button v-on:click="nextWeek()" class=" border px-4 bg-card hover:border-ring  py-2 rounded-3xl">></button>
                <button  v-on:click="goToCurrentWeek()" class=" rounded-3xl border border-border bg-card hover:border-ring   px-4 py-2  text-accent-foreground">Get Current Week</button>
            </div>
    </div>
    ...
    <section class=" flex  justify-around gap-2 ">
        ...
        <div :class="['flex flex-col items-center justify-center p-4 rounded-3xl  ', date.isToday ? 'bg-accent text-primary':' bg-accent/50']">
...
<!-- 
        ...
        :class="['  flex flex-col gap-2 p-2   min-h-[120px] border-2 border-dashed rounded-3xl', isStaffDragged?'border-amber-600':'']">
 ...
 <div    class='  flex flex-col gap-2 p-2   min-h-30 border-2 border-dashed rounded-3xl hover:border-ring'>
